        ...
        this.post('/register', ['PUBLIC'], async (req, res) => {
            try {
                ...
                if (password.length < 6) {
                    return res.sendUserError('La contraseña debe tener al menos 6 caracteres');
                }

                const normalizedEmail = email.trim().toLowerCase();
                if (await userModel.exists({ email: normalizedEmail })) {
                    return res.sendUserError('El email ya está registrado', 409);
                }
        ...
        this.post('/login', ['PUBLIC'], async (req, res) => {
            try {
                ...
                if (!user || !(await bcrypt.compare(password || '', user.password))) {
                    return res.sendUserError('Credenciales inválidas', 401);
                }
        ...
        this.post('/forgot-password', ['PUBLIC'], async (req, res) => {
            try {
                ...
                return res.sendSuccess({
                    message: 'Modo desarrollo: configurá MAIL_USER y MAIL_PASS para enviar el correo',
                    devToken: rawToken,
                    expiresInMinutes: config.resetTokenMinutes
                });
            } catch (error) {
                return res.sendServerError(error.message);
            }
        ...
        this.post('/reset-password', ['PUBLIC'], async (req, res) => {
            try {
                ...
                if (!token || !password || password.length < 6) {
                    return res.sendUserError('Token y una contraseña válida son obligatorios');
                }
                ...
                for (const previousHash of user.previousPasswords) {
                    if (await bcrypt.compare(password, previousHash)) {
                        return res.sendUserError('No podés reutilizar una contraseña anterior');
                    }
                }
